...
| 技能 | 干什么 |
| --- | --- |
| [opening-gate](skills/opening-gate/) | **开头闸门**。封面句 / 标题 / 开头五秒的发布前专项：封面句过三闸门（句里有人 / 牌亮了没有 / 是不是人话），开头零铺垫直给，素材硬度回检。附真实账号 16 条正负样本的点击率对照，和发布后的数据回流规则。 |
| [flow-check](skills/flow-check/) | **完播体检**。逐段找"观众在哪一句走掉"：接缝层 / 密度层 / 口感层（默读法）三层扫描；带画面的脚本强制加叙事层五项逐镜核（时间线 / 道具动线 / 悬念翻牌 / 情绪推进 / 声画对位）。句子层全绿不等于整条成立。 |
...
quality-gate（交付前：质检）★ 强制
        ↓
flow-check（完播体检）→ opening-gate（封面句 / 开头，发布前最后一关）
        ↓
